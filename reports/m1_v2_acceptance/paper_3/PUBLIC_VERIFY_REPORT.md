# Public M1 Verify Report: Anomaly Transformer: Time Series Anomaly Detection with Association Discrepancy

- paper_id: paper_3
- canonical_quality_status: PASS
- high_risk_count: 0
- medium_risk_count: 0
- low_risk_count: 0

## Metrics

- formula_count: 9
- latex_count: 9
- bbox_count: 9
- section_distribution: {'Abstract': 2, 'Introduction': 2, 'Related Work': 2, 'Method': 11, 'Experiments': 12, 'Conclusion': 6, 'References': 3}
- section_contradiction_count: 0
- all_formulas_in_Abstract_suspicious: False
- polluted_section_count: 0
- high_risk_count: 0
- medium_risk_count: 0
- low_risk_count: 0
- canonical_quality_status: PASS
- route: C Marker formula audit + PyMuPDF body fallback
- primary_parser: pymupdf
- raw_formula_text_count: 0
- crop_exists: 9
- overlay_exists: 5
- canonical_match: True
- runtime_seconds: 0.414
- runtime_device: CPU/cached artifacts
- peak_vram_estimate: not measured
- ollama_json_valid: 0
- ollama_json_invalid: 0
- ollama_retry: 0
- ollama_timeout: 0
- ollama_changed_by_count: 0
- core_formula_coverage: {'Prior-Association': 'FOUND_TEXT', 'Series-Association': 'FOUND_TEXT', 'AssDis(P,S;X)': 'FOUND_LATEX', 'AnomalyScore': 'FOUND_TEXT'}