...
| parsed_document.json | v2 升级 | PyMuPDF fallback 质量低 | ParserAdapter 统一接口 | 否 |
| evidence_index.json | v2 升级 | block-level evidence | passage-level + claim extraction | 否 |
| paper_skeleton.json | v2 升级 | rule-based 提取保守 | LLM-enhanced extraction | 否 |
| paper_card.json | v2 升级 | rule-based baseline | evidence-constrained LLM explainer | 否 |
| formula_cards.json | v2 升级 | generic symbol dictionary | paper-context symbol grounding | 否 |
| teaching_cards.json | v2 升级 | rule-based baseline | evidence-constrained five-layer | 否 |
...
**结论**：所有 artifact 保留，不阻塞 Phase 12。v2 升级可在后续 Phase 进行。
