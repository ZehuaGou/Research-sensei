...
5. Source download
6. Material normalization (three-pipeline architecture):
   - **Body pipeline**: MarkItDown (default) → PyMuPDF (text fallback) → optional Marker (text path)
   - **Formula pipeline**: MarkerDocumentFormulaDetector (`build_document()` → Equation blocks with bbox) → FormulaSlot → FormulaCropper (PyMuPDF crop)
   - **FormulaMerger**: merges body sections + formula slots → canonical_paper.md
