...
**IMPORTANT**: Current code uses `MinerUPdfAdapter` with `magic_pdf.tools.common.do_parse` (old MinerU CLI). This is NOT the same as `mineru-vl-utils` + `opendatalab/MinerU2.5-Pro-2604-1.2B`. Marker is now fallback/audit baseline, not primary parser.
