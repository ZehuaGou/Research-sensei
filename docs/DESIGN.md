...
7. FormulaRegionDetector
8. FormulaOCRAdapter
9. `canonical_paper.md` generation
10. `m2_ready` gate
...
````markdown
<!-- formula_id: eq1 | origin: source_latex | section: Method | page: 4 | bbox: [x1,y1,x2,y2] | ocr_status: not_required -->
```latex
\mathcal{L} = ...
```
````

`formula_origin` 取值固定为 `source_latex`、`parser_latex`、`ocr_latex`、`reconstructed`、`unknown`。

**FormulaRegionDetector**:
- 输入：PDF parser / layout parser / MinerU / Marker 的 block、page image、layout metadata。
- 输出：`formula_id`、`formula_bbox`、`formula_page`、`section`、`detector_confidence`、`warnings`。
- 失败条件：无 bbox、bbox 越界、与正文 section 无法对齐、检测置信度低。
- gate：无可靠公式区域时可继续生成 `canonical_paper.md`，但公式能力标记为 degraded。
- 当前状态：DOC_DESIGNED / NOT_IMPLEMENTED。

**FormulaOCRAdapter**:
- 输入：公式区域图像、bbox、page、上下文、OCR 配置。
- 输出：`formula_latex`、`formula_origin=ocr_latex`、`formula_ocr_status`、`ocr_confidence`、`warnings`。
- 候选实现：pix2tex / LaTeX-OCR，通过 adapter 接入。
- 失败条件：超时、显存不足、OCR 低置信、输出不可解析、与上下文冲突。
- 运行策略：`formula_ocr_enabled: true`，`default_formula_ocr_mode: on_demand`，`max_formula_ocr_per_paper: 3`，`max_formula_ocr_batch: 10`，`formula_ocr_timeout_seconds: configurable`。
- 触发条件：用户点击解释公式、M2 选为核心 top-K、parser 检测到公式区域但缺少可靠 LaTeX、parser_latex 与上下文冲突、深度精读模式开启。
- 当前状态：DOC_DESIGNED / NOT_IMPLEMENTED。
...
| MinerUAdapter / MarkerAdapter / DoclingAdapter | M1 material normalization | EVALUATED_IN_DOC, NOT_IMPLEMENTED |
