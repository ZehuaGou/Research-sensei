...
| 编号 | 模块 | 职责 |
|------|------|------|
| M1 | 论文搜索、综述优先、方向探索、最佳原始材料获取、阅读计划、seed paper expansion | 搜索、验证、best available source 获取、筛选、排序、生成阅读计划和方向框架 |
| M2 | 单篇论文精读、综述论文精读、证据链、公式/符号/方法机制讲解 | 根据 preferred_m2_input 选择最佳解析路径、证据链路、讲解生成、质量审计、状态门控 |
...
M1 的职责不只是下载 PDF，而是获取 best available source。

**M1 source priority**:
1. LaTeX source / arXiv source
2. structured HTML / XML / publisher HTML
3. PDF
4. metadata only

M2 的职责不只是解析 PDF，而是根据 M1 提供的 `preferred_m2_input` 选择最佳解析路径。

**M2 parser priority**:
1. LaTeX source parser
2. structured HTML / XML parser
3. formula-aware PDF parser
4. structure-aware PDF parser
5. low-confidence PDF text fallback

PDF→LaTeX / PDF→Markdown 是 source 不可得时的 fallback，不是首选路线。如果 LaTeX source 可用，M2 应优先使用 source 解析公式、符号、section、citation 和 bibliography。
...
| LaTeXSourceParser | M2.1 | DOC_DESIGNED, NOT_IMPLEMENTED |
| MinerUAdapter / DoclingAdapter | M2.1 | EVALUATED_IN_DOC, NOT_IMPLEMENTED |
| Source-aware parser selection | M2.1 | DOC_DESIGNED, NOT_IMPLEMENTED |
