...
- M1 v2 primary-route acceptance is verified on two new unseen papers in `reports/m1_v2_mineru_primary_acceptance/`; fallback reports are debug/audit evidence only.
- Dense raw-only formula sets (`formula_count >= 5` and `latex_count == 0`) must not be marked formula-understanding-ready.
