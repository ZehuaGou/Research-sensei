...
- MinerU2.5-Pro (mineru-vl-utils + opendatalab/MinerU2.5-Pro-2604-1.2B) is the primary M1 parser. Marker is fallback/audit baseline. PyMuPDF/MarkItDown are lightweight fallback/debug.

### M1 canonical pipeline invariants

- MinerU2.5-Pro via mineru-vl-utils is the primary M1 parser.
- Marker is fallback/audit baseline.
- Ollama is an optional structured refiner, default OFF.
- Ollama must not modify latex, bbox, page, or source identity.
- M1 gate blocks: all-formulas-in-Abstract, section contradiction, source/title mismatch, missing latex/crop/overlay, dense raw-only formulas.
- References formulas are excluded from formula understanding.
- Primary-route acceptance is verified only for selected real papers regenerated
  with the current M1 pipeline. Broad multi-paper MinerU acceptance remains
  pending, and generated reports are not committed.
...
| M2.3 讲解生成 | [M2_3_PAPER_UNDERSTANDING.md](development/M2_3_PAPER_UNDERSTANDING.md) | `src/researchsensei/paper_card.py`, `formula_card.py`, `teaching_card.py`, `*_baseline.py` |
