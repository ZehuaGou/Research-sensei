...
- `python -m pytest -q` 全部通过（包括 tests_live）
- 缺 env/key/network 时 tests_live 必须失败，不能 skip
...
real LLM smoke 通过 `python -m pytest -q` 默认运行（tests_live 已包含在 testpaths 中）。缺 API key 时必须失败，不能 skip。
...
- `python -m pytest -q` 默认运行 tests_live
- 缺 key / 缺网络 = 测试失败，不能 skip
- 必须有 token limit
- 必须有 cost limit
- 必须限制样例数量
