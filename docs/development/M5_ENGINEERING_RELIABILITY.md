...
定义后端 pytest 默认规则，确保默认测试不联网、不真实调用 LLM。
...
- 默认 pytest 不联网
- 默认 pytest 不真实调用 LLM
- 外部服务测试默认使用 `httpx.MockTransport`
- LLM 测试默认使用 fake/mock client
- live / network / llm / slow 测试必须通过 marker 排除
- 普通 CI 不跑 live / real LLM 测试
...
- 默认 `python -m pytest -q` 不联网、不真实调用 LLM
- 所有外部服务测试使用 mock
- 所有 LLM 测试使用 fake client

### 当前实现状态

- 已实现：481 tests，全部默认通过
- MockTransport 已用于 HTTP 测试
- MockLLMClient 已用于 LLM 测试
...
定义真实 LLM smoke 测试规则，确保 opt-in、受控、不影响普通 CI。
...
| 测试 | 断言 |
|------|------|
| test_llm_smoke_opt_in | 默认不执行，RUN_LLM_TESTS=1 才执行 |
...
- 已实现：`tests_live/`
- 已实现：`scripts/run_live_eval.py`
- 已实现：`src/researchsensei/live_eval.py`
...
- 当前一次 live 验证：M1 OpenAlex 返回候选；M2 MiMo 调用 3 次，token 记录成功；teaching card 可降级但 report 如实记录
