...
| 输出 | 源文件（LaTeX source / PDF）或 metadata-only fallback + 获取状态 |
...
- 下载/获取失败记录到 CandidatePaper 的 warnings
- source_status.json 表达获取状态（success / degraded / failed）
...
- M1.3 未实现：论文原始材料获取（arXiv source fetch / PDF download / metadata-only fallback）方案已设计，代码未写
- M1.4 已实现：SelectionService（deduplicate + scoring + reading plan）
- M1.5 已实现：DirectionRunner（编排 M1 全链路）
...
- M1.3 论文原始材料获取代码未实现（方案已设计：arXiv source fetch / PDF download / metadata-only fallback）
- M1.3 与 M2.0 SourceResolver 的衔接细节（M1.3 获取原始材料，M2.0 负责解析入口）
